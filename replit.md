...
- Web search features (PESTEL, análise competitiva, Contexto Macro) use `openaiSearch` — a second OpenAI client pointing to `api.openai.com` (not Azure) initialized from `OPENAI_API_KEY`. Uses `responses.create` with `gpt-4o-mini-search-preview` + `web_search_preview`. Falls back to Azure chat completions if `OPENAI_API_KEY` is absent.
