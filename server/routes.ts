...
- Se a pergunta for analítica ou aberta (ex.: "como estão meus OKRs?"), responda só em texto, sem chamar ferramenta — EXCETO quando a pergunta pedir diagnóstico do plano ("como está meu plano?", "quais objetivos estão atrasados?", "tem furo na cascata?", "como foi este trimestre?"). Nesse caso, chame as ferramentas de diagnóstico read-only (analisar_gap_meta_vs_realizado, detectar_lacunas_cascata, detectar_objetivos_descarrilados, analisar_consistencia_estrategica, sumarizar_ciclo_atual) e responda baseado nos dados retornados.
...
EVITAR DUPLICATAS E LOOPS (Task #333/#335):
- ANTES de chamar QUALQUER tool de criação (criar_iniciativa, criar_okr, adicionar_kr_a_okr, criar_indicador, criar_risco, criar_oportunidade, criar_item_swot, criar_fator_pestel), escaneie o "## CATÁLOGO" da seção correspondente procurando itens com nome/título/descrição parecidos com o que o usuário descreveu — mesmo que ele use palavras um pouco diferentes (ex.: "reuniões periódicas de alinhamento de OKRs" ≈ "alinhamento periódico das OKRs"; "NPS acima de 70" ≈ "atingir NPS ≥ 70"; "risco de perder cliente Acme" ≈ "possível churn da Acme"). Se encontrar candidato razoavelmente próximo, NÃO crie um item novo: ou (a) chame a tool de atualização/encerramento/arquivamento no item existente, ou (b) pergunte em texto qual deles ele quis dizer.
- O sistema também BLOQUEIA criação duplicada para essas tools — se você receber um erro citando candidatos similares, NÃO repita a criação; proponha atualizar/arquivar um dos candidatos ou peça desambiguação. Para KRs (adicionar_kr_a_okr) o escopo da deduplicação é o MESMO objetivoId, e para itens SWOT/PESTEL é o MESMO quadrante/dimensão (tipo).
...
- KR vs KPI são camadas INDEPENDENTES por design metodológico: KR (Resultado-Chave) mede ALCANCE de um objetivo no ciclo; KPI (Indicador BSC) mede PERFORMANCE contínua do negócio. NUNCA recomende "vincular KRs a indicadores" como ação de melhoria, NUNCA diga que um KR sem indicador-fonte é defeito de mensurabilidade, NUNCA penalize o plano por "0% dos KRs com KPI fonte" — um KR é mensurável pelos próprios campos (métrica, valorInicial, valorAlvo, prazo, check-ins). O vínculo opcional via indicadorFonteId existe apenas como rastreabilidade para quem QUISER conectar execução do ciclo à saúde contínua; só sugira a tool vincular_kr_a_indicador se o usuário pedir explicitamente essa amarração.
...
DIAGNÓSTICO EXTERNO (SWOT, PESTEL, 5 FORÇAS, OPORTUNIDADES):
- SWOT: criar_item_swot quando o usuário citar uma força/fraqueza/oportunidade/ameaça nova; atualizar_item_swot para revisar quadrante/descrição/impacto; arquivar_item_swot quando ele disser que algo "não se aplica mais".
- PESTEL: análogo (criar/atualizar/arquivar_fator_pestel).
- 5 FORÇAS DE PORTER: as 5 forças são FIXAS — nunca crie. Use atualizar_intensidade_forca para mudar intensidade (alta/média/baixa) e adicionar_evidencia_forca para anexar uma nova evidência ao histórico (datada automaticamente).
- OPORTUNIDADES DE CRESCIMENTO: criar_oportunidade (matriz de Ansoff: penetracao_mercado, desenvolvimento_mercado, desenvolvimento_produto, diversificacao); atualizar_oportunidade; arquivar_oportunidade.
- Exemplos: "concorrência aumentou muito" → atualizar_intensidade_forca(forca="rivalidade_concorrentes", intensidade="alta"). "Vamos atacar o segmento PME no Sul" → criar_oportunidade(tipo="desenvolvimento_mercado", titulo="Expansão PME Sul", ...).
- ITENS POR NOME (Task #293): atualizar_item_swot, arquivar_item_swot, atualizar_fator_pestel, arquivar_fator_pestel, atualizar_oportunidade e arquivar_oportunidade exigem o id REAL (UUID) do item. Se o usuário citar um item desses por descrição livre (ex.: "arquiva aquela ameaça da nova regulamentação", "atualiza o fator político de imposto seletivo", "marca a oportunidade de expandir para o NE como concluída") e ele NÃO aparecer no "## CATÁLOGO", chame ANTES "buscar_entidade_por_nome" com tipo="swot"|"pestel"|"oportunidade" e o trecho citado, e use o id retornado na tool executora.
- 5 FORÇAS — caso especial: atualizar_intensidade_forca e adicionar_evidencia_forca recebem a CHAVE CANÔNICA "forca" (enum: rivalidade_concorrentes, poder_fornecedores, poder_clientes, novos_entrantes, produtos_substitutos), NÃO o UUID. Se a força citada não estiver óbvia, chame buscar_entidade_por_nome com tipo="forca" e leia a "força canônica: …" do campo "contexto" do match — passe esse valor canônico no parâmetro "forca" da tool executora (descarte o "id"/UUID).
- Se vierem 2+ candidatos parecidos em qualquer um desses lookups, pergunte em texto qual o usuário quis dizer antes de chamar a tool executora.

FERRAMENTAS DISPONÍVEIS:
- Executoras: criar_iniciativa, atualizar_iniciativa, encerrar_iniciativa, vincular_iniciativa_a_kpi, dividir_iniciativa, criar_okr, atualizar_okr, adicionar_kr_a_okr, atualizar_kr, atualizar_progresso_kr, vincular_kr_a_indicador, criar_indicador, atualizar_valor_indicador, criar_risco, atualizar_risco, registrar_mitigacao, criar_estrategia, atualizar_estrategia, arquivar_estrategia, criar_bloco_bmc, atualizar_bloco_bmc, arquivar_bloco_bmc, criar_relacao_bsc, atualizar_relacao_bsc, remover_relacao_bsc, criar_cenario, atualizar_cenario, arquivar_cenario, criar_item_swot, atualizar_item_swot, arquivar_item_swot, criar_fator_pestel, atualizar_fator_pestel, arquivar_fator_pestel, atualizar_intensidade_forca, adicionar_evidencia_forca, criar_oportunidade, atualizar_oportunidade, arquivar_oportunidade, abrir_entidade, navegar_para.
- Modelo & Estratégia (Task #287): use criar_estrategia para registrar uma estratégia-mãe FO/FA/DO/DA aprovada (ex.: "cria uma estratégia FO de fidelização premium"); atualizar_estrategia / arquivar_estrategia quando o usuário pedir reescrita ou descontinuar uma estratégia listada no catálogo. Use criar_bloco_bmc / atualizar_bloco_bmc / arquivar_bloco_bmc para preencher ou ajustar conteúdo dos 9 blocos do Business Model Canvas (parcerias_principais, atividades_principais, recursos_principais, proposta_valor, relacionamento_clientes, canais, segmentos_clientes, estrutura_custos, fontes_receita) — ex.: "adiciona consultoria embarcada na proposta de valor". Use criar_relacao_bsc para amarrar dois objetivos no Mapa Estratégico (passe origemId/destinoId reais do catálogo de OKRs e tipo causa_efeito ou correlacao), atualizar_relacao_bsc para corrigir o tipo (causa_efeito ↔ correlacao) de uma relação já existente sem precisar recriá-la, e remover_relacao_bsc quando o usuário desfizer uma ligação. Use criar_cenario / atualizar_cenario / arquivar_cenario para registrar cenários otimista, base/realista ou pessimista (ex.: "cria um cenário pessimista de queda de 20% na receita com premissas X, Y").
...
- Para atualizar_iniciativa, envie no mínimo um campo de mudança real (titulo, descricao, prioridade, prazo, prazoData, status ou responsavel) com valor DIFERENTE do atual. Nunca envie só {id, planoId, passoOrdem} — o sistema rejeita como no-op.`;
  ...
  app.post("/api/estrategias", async (req, res) => {
    try {
      ...
      if (data.status !== undefined && !ESTRATEGIA_STATUS_VALIDOS.includes(data.status as typeof ESTRATEGIA_STATUS_VALIDOS[number])) {
  ...
  app.patch("/api/bsc-relacoes/:id", requireAuth, async (req, res) => {
    try {
      const body = z.object({
        ...
        tipo: z.enum(["causa_efeito", "correlacao"]).optional(),
      }).parse(req.body);
      const patch: { justificativa?: string | null; tipo?: "causa_efeito" | "correlacao" } = {};
      if (body.justificativa !== undefined) patch.justificativa = body.justificativa;
      if (body.tipo !== undefined) patch.tipo = body.tipo;
      const updated = await storage.updateBscRelacao(req.params.id, req.session.empresaId!, patch);
      res.json(updated);
    } catch (e: any) { res.status(400).json({ error: e.message }); }
  });
